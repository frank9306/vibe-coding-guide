...
if ($null -ne $manifest) {
    ...
    if ($manifest.instructionLanguage -ne 'en' -or $null -eq $personalization -or $personalization.required -ne $true -or $personalization.allowDefaults -ne $true -or $personalization.mode -ne 'agent-guided-review-merge' -or $personalization.writeLanguage -ne 'en' -or $personalization.preserveLiteralValues -ne $true -or $targetResolution.preferredEnvironmentVariable -ne 'CODEX_HOME' -or $targetResolution.relativePath -ne 'AGENTS.md' -or $targetResolution.requireAgentHome -ne $true -or $targetResolution.allowUserHomeFallback -ne $false) {
